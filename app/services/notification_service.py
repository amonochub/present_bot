...
from app.db.notification import Notification, NotificationType, NotificationStatus
...
class NotificationService:
    ...
    @staticmethod
    async def create_task_reminder(task_id: int, user_id: int, hours_before: int = 24) -> Notification:
            ...
            result = await session.execute(
                select(Notification).where(
                    and_(
                        ...
                        Notification.status.in_([NotificationStatus.READ, NotificationStatus.FAILED])
                    )
                )
            )
