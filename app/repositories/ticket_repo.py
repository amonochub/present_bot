...
from app.middlewares.metrics import increment_tickets, decrement_tickets
from typing import List, Optional

async def create_ticket(author_id: int, title: str, file_id: str | None) -> Ticket:
...
async def list_all() -> List[Ticket]:
    async with AsyncSessionLocal() as s:
        rows = await s.scalars(select(Ticket).order_by(Ticket.created_at.desc()))
        return list(rows)

async def set_status(tkt_id: int, status: Status) -> bool:
    try:
        async with AsyncSessionLocal() as s:
            await s.execute(update(Ticket).where(Ticket.id == tkt_id)
