...
from app.keyboards.main_menu import menu, get_back_btn
...
async def back_to_main_cb(call: CallbackQuery, state, lang: str):
    await state.clear()
    await call.message.edit_text("🏠 Вы вернулись в демо-меню",
...
async def next_step(msg: Message, state, lang: str):
    ...
    if idx >= len(TOUR_ROLES):
        await state.clear()
        await msg.answer("🚀 Тур завершён! Вы снова в демо-меню.",
                         reply_markup=menu("super", lang))
        return
    ...
    await msg.answer(_tour_text(role), parse_mode="HTML",
