...
from app.db.notification import NotificationStatus
...
async def show_notifications(msg: Message):
    ...
    async with AsyncSessionLocal() as session:
        ...
        notifications = await NotificationService.get_user_notifications(
            user_id=user.id, limit=5, unread_only=True
        )
        ...
        keyboard.append(
            [InlineKeyboardButton("⚙️ Настройки", callback_data="notification_settings")]
        )
...
async def mark_notification_read(call: CallbackQuery):
    ...
    if success:
        ...
        await call.message.edit_text(
            call.message.text + "\n\n✅ **Отмечено как прочитанное**", parse_mode="Markdown"
        )
    else:
...
async def back_to_main(call: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if user:
            await call.message.edit_text("🏠 Главное меню", reply_markup=menu(user.role, "ru"))
        else:
