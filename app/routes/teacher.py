...
async def save_note(msg: Message, state, lang: str):
    try:
        student, text = msg.text.strip().split(maxsplit=2)[:2], " ".join(
            msg.text.strip().split(maxsplit=2)[2:]
        )
...
async def media_finish(msg: Message, state, lang: str):
    data = await state.get_data()
    user = await me(msg.from_user.id)
    await media_repo.media_create(user.id, data["event_date"], msg.text.strip(), data["file_id"])
    await state.clear()
