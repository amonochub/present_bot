...
from app.middlewares.ux import UnknownCommandMiddleware, FallbackMiddleware
...
async def seed_demo(conn):
    ...
    # --- медиа-заявка ---
    media = [
        MediaRequest(author_id=teacher1,
    ...
    psych = [
        PsychRequest(from_id=student2,
                     text="Меня дразнят одноклассники, не хочу идти в школу",
                     status=Status.open),
        PsychRequest(from_id=student3,
    ...
    tasks = [
        Task(title="Подготовить отчёт по успеваемости",
             description="Собрать данные по всем классам за текущий месяц",
             status=TaskStatus.PENDING, author_id=1, assigned_to_id=teacher1,
             deadline=date.today() + timedelta(days=3)),  # author_id=1 (директор)
        Task(title="Обновить расписание в МЭШ",
             description="Внести изменения в электронное расписание",
             status=TaskStatus.PENDING, author_id=1, assigned_to_id=teacher2,
             deadline=date.today() - timedelta(days=1)),  # author_id=1 (директор) - просрочено
    ]

    # --- массовые рассылки админа ---
    broadcasts = [
        Broadcast(author_id=6,  # admin01
...
async def fsm_login(m: Message, state: FSMContext, lang: str):
    ...
    if current_state == "await_login":
        ...
        if user:
            ...
            await m.answer(t("common.auth_success", lang),
