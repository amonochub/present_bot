...
class TestConfigSecurity:
    ...
    def test_database_password_validation(self):
        ...
        with patch.dict(
            ...
            with pytest.raises(
                ValueError, match="Database password must be at least 8 characters long"
            ):
                Settings()
    ...
    def test_environment_variables_not_in_code(self):
        ...
        assert (
            not hasattr(settings, "TELEGRAM_TOKEN")
            or settings.TELEGRAM_TOKEN != "your_bot_token_here"
        )
        ...
        assert (
            not hasattr(settings, "GLITCHTIP_DSN")
            or settings.GLITCHTIP_DSN is None
            or "CHANGE_ME" not in str(settings.GLITCHTIP_DSN)
        )
