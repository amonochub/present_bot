...
class TestXSSProtection:
    ...
    def test_user_input_validation(self):
        ...
        safe_chars = "абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ0123456789 .,!?()-"
        dangerous_chars = "<>\"'&"
    ...
    def test_callback_data_safety(self):
        ...
        for callback in dangerous_callbacks:
            ...
            has_dangerous = (
                "<script>" in callback or
                "javascript:" in callback or
                "<iframe" in callback or
                "onerror=" in callback or
                "onload=" in callback or
                "<svg" in callback
            )
