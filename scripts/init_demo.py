...
async def init_demo_user():
    ...
    # Создаем сессию
    async_session = sessionmaker(engine, class_=asyncio.AsyncSession, expire_on_commit=False)

    async with async_session() as session:
        # Проверяем, существует ли уже демо-пользователь
        result = await session.execute("SELECT id FROM users WHERE login = 'demo01'")
